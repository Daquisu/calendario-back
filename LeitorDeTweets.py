...
class Tweet:
    ...
    def unificarTexto(self):
        for frase in self.frases[1:]:
            self.frases[0] += frase
...
def verificarSemelhanca(texto1, texto2, textosDoDiaOriginal):
    # Podemos substituir o código abaixo trocando o parâmetro
    # tweets para textosDoDia
    ...
    textosDoDia = textosDoDiaOriginal[:]
    textosDoDia.remove(texto1) 
...
def semelhancaDeTweets(twt1, twt2, textosDoDia):
    tamanhoTotal = 0
    ...
    for frase1 in twt1['frases']:
        for frase2 in twt2['frases']:
            semelhanca += (verificarSemelhanca(frase1, frase2, textosDoDia)) * (len(frase1) + len(frase2))
...
'''def main():
    caminhoJson = "jsonTweetsTeste.json"
    caminhoTxt = "TweetsDoDia.txt"
    caminhoJsonSaida = "Tweets.json"
